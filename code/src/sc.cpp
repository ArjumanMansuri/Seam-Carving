...
bool seam_carving(Mat& in_image, int new_width, int new_height, Mat& out_image, int* verticalMatrix, int* horizontalMatrix) {
	...
	return seam_carving_trivial(in_image, new_width, new_height, out_image, verticalMatrix, horizontalMatrix);
}


// seam carves by removing trivial seams
bool seam_carving_trivial(Mat& in_image, int new_width, int new_height, Mat& out_image, int* verticalMatrix, int* horizontalMatrix) {
	...
	while (iimage.rows != new_height || iimage.cols != new_width) {
		...
		if (iimage.rows > new_height) {
			reduce_horizontal_seam_trivial(iimage, oimage, horizontalMatrix);
			iimage = oimage.clone();
		}
		// vertical seam if needed
		if (iimage.cols > new_width) {
			reduce_vertical_seam_trivial(iimage, oimage, verticalMatrix);
			iimage = oimage.clone();
		}
	}
...
bool reduce_horizontal_seam_trivial(Mat& in_image, Mat& out_image, int* horizontalMatrix) {
...
bool reduce_vertical_seam_trivial(Mat& in_image, Mat& out_image, int* verticalMatrix) {
	// retrieve the dimensions of the new image
